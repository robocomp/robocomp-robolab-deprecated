...
int sevilla2Comp::run(int argc, char* argv[])
{
	...
	a.setApplicationName("phonnon");
	// Remote server proxy access example
	// RemoteComponentPrx remotecomponent_proxy;
	JuegoPrx juego_proxy;
...
UrsusArmPrx ursusarm_proxy;
	...
	try
	{
		juego_proxy = JuegoPrx::uncheckedCast( communicator()->stringToProxy( getProxyString("JuegoProxy") ) );
	}
	catch(const Ice::Exception& ex)
	{
	...
	rInfo("JuegoProxy initialized Ok!");
	mprx["JuegoProxy"] = (::IceProxy::Ice::Object*)(&juego_proxy);//Remote server proxy creation example
	try
	{

...
void Worker::updateTemperature()
{
	...
	try {
		...
		//s3 pulso
		lcdPulso->display( mstateMap["s3"].temperature );
		//s4 codo
		lcdCodo->display( mstateMap["s4"].temperature );
	}
...
void Worker::habla()
{
  try 
  {
	speech->say(textEdit->toPlainText().toStdString());
  }
  catch (Ice::Exception e) 
  {

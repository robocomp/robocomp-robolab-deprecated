...
Juego1::Juego1(osg::ref_ptr< osg::Camera > _cam, osg::ref_ptr< osgART::Tracker > _tracker, RoboCompArmrocio::ArmrocioPrx _arm, RoboCompSpeech::SpeechPrx _speech, QFrame* parent)
{
  cam=_cam;
  tracker=_tracker;
  arm=_arm;
...
void Juego1::armArriba(bool b)
{
	try {
		arm->setPose(RoboCompArmrocio::game1Up);
	}
	catch (Ice::Exception e ) {
		qDebug()<<"Error talking to Arm"<<e.what();
	}
}
void Juego1::armReposo(bool b)
{
	try {
...
void Juego1::armAbajo(bool b)
{
	try {
		arm->setPose(RoboCompArmrocio::game1Down);
	}
	catch (Ice::Exception e ) {
		qDebug()<<"Error talking to Arm"<<e.what();
	}
...
osg::Geode* Juego1::ball (float size, osg::Vec4 color)
{
 ...
 */
osg::Geode* Juego1::testCube(float size, osg::Vec4 color) {
...
void Juego1::updateBola (osg::Vec3 centro, osg::Vec4 color)
{

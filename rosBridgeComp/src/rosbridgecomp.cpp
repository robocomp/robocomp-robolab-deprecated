...
using namespace RoboCompSpeaker;


class rosBridgeComp : public RoboComp::Application
{
private:
...
void rosBridgeComp::initialize()
{
	// Config file properties read example
	// configGetString( PROPERTY_NAME_1, property1_holder, PROPERTY_1_DEFAULT_VALUE );
	// configGetInt( PROPERTY_NAME_2, property1_holder, PROPERTY_2_DEFAULT_VALUE );
}
int rosBridgeComp::run(int argc, char* argv[])
{
#ifdef USE_QTGUI
	...
	try
	{
		speaker_proxy = SpeakerPrx::uncheckedCast( communicator()->stringToProxy( getProxyString("SpeakerProxy") ) );
	}
	catch(const Ice::Exception& ex)
	{
	...
	rInfo("SpeakerProxy initialized Ok!");
	mprx["SpeakerProxy"] = (::IceProxy::Ice::Object*)(&speaker_proxy);
	
	GenericWorker *worker = new SpecificWorker(mprx);
	RosAdapter *ros = new RosAdapter((SpecificWorker*)worker);
	ros->start();

...
namespace osgDTE {
  ...
  void modeloNOMAD::crearsegmentos()
  {

    // Base
    jBase.trf->setName("jBase");
    jBase.trf->setMatrix(osg::Matrix::translate(0.0,0.0,0.0f)); // la base del NOMAD descansa en el suelo
    //jBase.trf->setMatrix(osg::Matrix::translate(0.0,0.0,237.0f));
    this->addChild(jBase.trf.get());
    osg::ref_ptr<osg::Node> node = osgDB::readNodeFile(string("../../data/NOMAD200Base.osg"));
    ...
    node = osgDB::readNodeFile(string("../../data/NOMAD200Cuerpo.osg"));
    ...
    node = osgDB::readNodeFile(string("../../data/NOMAD200CabezaVidere.osg"));
    ...
    // Hombro izquierdo
    jHombroIzq.trf->setName("jHombroIzq");
    jHombroIzq.trf->setMatrix(osg::Matrix::rotate(osg::inDegrees(-90.0f),0.0f,0.0f,1.0f) *
			      osg::Matrix::translate(20.0f,0.0f,82.0f));
    jCuerpo.trf->addChild(jHombroIzq.trf.get());
    node = osgDB::readNodeFile(string("../../data/NOMAD200Hombro.osg"));
    MatrixTransform* intermedia3 = new MatrixTransform;
    intermedia3->setMatrix(osg::Matrix::rotate(inDegrees(90.0f),0.0f,1.0f,0.0f));
    if (node.valid()) {intermedia3->addChild(node.get()); jHombroIzq.trf->addChild(intermedia3);}
    jHombroIzq.dirsegmento.set(0.0f,0.0f,115.0f); // ojo, igual falta un desplazamiento (x=-10 en JP's)  
    jHombroIzq.nodepath.push_back(jBase.trf.get());
    jHombroIzq.nodepath.push_back(jCuerpo.trf.get());  
    jHombroIzq.nodepath.push_back(jHombroIzq.trf.get());

    jHombroIzq.trf->addChild(crearEjeCoordenadas(3.0));
    ...
    node = osgDB::readNodeFile(string("../../data/NOMAD200Brazo.osg"));
    ...
    node = osgDB::readNodeFile(string("../../data/NOMAD200Antebrazo.osg"));
    ...
    jAntebrazoIzq.dirsegmento.set(0.0f,0.0f,29.05f); // OJO ESTE NO COINCIDE CON EL DEL NOMAD (hay que retocar el modelo 3D)...
    ...
    // Hombro derecho
    jHombroDer.trf->setName("jHombroDer");
    jHombroDer.trf->setMatrix(osg::Matrix::rotate(osg::inDegrees(90.0f),0.0f,0.0f,1.0f) *
			      osg::Matrix::translate(-20.0f,0.0f,82.0f));
    jCuerpo.trf->addChild(jHombroDer.trf.get());
    node = osgDB::readNodeFile(string("../../data/NOMAD200Hombro.osg"));
    MatrixTransform* intermedia6 = new MatrixTransform;
    intermedia6->setMatrix(osg::Matrix::rotate(inDegrees(90.0f),0.0f,1.0f,0.0f));
    if (node.valid()) {intermedia6->addChild(node.get()); jHombroDer.trf->addChild(intermedia6);}
    jHombroDer.dirsegmento.set(0.0f,0.0f,1.0f); // ojo, igual falta un desplazamiento (x=-10 en JP's)  
    jHombroDer.nodepath.push_back(jBase.trf.get());
    jHombroDer.nodepath.push_back(jCuerpo.trf.get());  
    jHombroDer.nodepath.push_back(jHombroDer.trf.get());
    ...
    jAntebrazoDer.dirsegmento.set(0.0f,0.0f,29.05f); // OJO ESTE NO COINCIDE CON EL DEL NOMAD (hay que retocar el modelo 3D)...
  ...
  modelo::joint* modeloNOMAD::accedeJoint(std::string nombre_joint)
  {
    ...
    cout<<"AVISO: No se ha encontrado el joint "<<nombre_joint<<" en el modelo del NOMAD"<<endl;
    return 0;
  }

  void modeloNOMAD::mueveModelo(const osg::Vec3d& donde)
  {
    jBase.trf->setMatrix(osg::Matrix::translate(donde));
  }
  
  void modeloNOMAD::reset()
  {
    jBase.trf->setMatrix(osg::Matrix::translate(0.0,0.0,0.0)); // La base del NOMAD apoya en el suelo
  ...
  void modeloNOMAD::linksMovidos(int i, std::vector<std::string>* lista)
  {
    switch (i)
      {
      case 19:
	...
	break;
      ...
      default:
	cerr<<"Índice de articulación no válido para el robot NOMAD-1"<<endl;
	break;
      }

  }

  void modeloNOMAD::limitesDefecto()
  {
    ...
    jBrazoDer.limiteeje[1].bajo = -95;  // RARM_JOINT[2] // Motor 2 del NOMAD
    jBrazoDer.limiteeje[1].alto = 0;
    jBrazoDer.limiteeje[0].bajo = -90;  // RARM_JOINT[1] // Motor 1 del NOMAD
  ...
  void modeloNOMAD::colisionesDefecto()
  {
    ...
    incluirColision(&jAntebrazoDer,&jBase);  
    incluirColision(&jAntebrazoDer,&jCuerpo);  
    incluirColision(&jAntebrazoDer,&jCabeza);  
    incluirColision(&jAntebrazoDer,&jBrazoIzq);    
    // Colisiones del brazo izdo
    incluirColision(&jBrazoIzq,&jBase);  
    incluirColision(&jBrazoIzq,&jCuerpo);   
    incluirColision(&jBrazoIzq,&jCabeza);  
    // Colisiones del brazo dcho
    incluirColision(&jBrazoDer,&jBase);   
    incluirColision(&jBrazoDer,&jCuerpo);   
    incluirColision(&jBrazoDer,&jCabeza);
  }

// Comprobación de ángulos para el modelo de NOMAD
  double modeloNOMAD::enlimitesInt(int articulacion, double angulo)
  {
    switch (articulacion)
      {
      case 6:
	...
	break;
      ...
      default:
	cerr<<"modeloNOMAD: Indice de articulacion no valido"<<endl; 
	break;
      } 
  ...
  // Comprobación de ángulos para el NOMAD
  bool modeloNOMAD::enlimites(int articulacion, double angulo)
  {
    switch (articulacion)
      {
      case 6:
  ...
  void modeloNOMAD::actualizadatos(double intervalo)
  {
  ...
  std::vector<double> modeloNOMAD::apuntaIKBrazoDerAn(const osg::Vec3d& donde)
  {
    ...
    // OFFSETS PARA EL NOMAD-1
    // Como los ejes de giro del NOMAD son un poco distintos a los de coordenadas locales, y como
    // los ángulos iniciales también son distintos, es necesario utilizar offsets para adaptar 
    // los valores anteriores al NOMAD-1
  ...
  std::vector<double> modeloNOMAD::apuntaIKBrazoIzqAn(const osg::Vec3d& donde)
  {
    // Matrices de resultados parciales
    osg::Matrix homb;
    osg::Matrix cod;
  ...
  // Función para mover el hombro a partir del giro en X del brazo, relativo al plano sagital (inicial)
  void modeloNOMAD::mueveHombroIzq()
  {
    Vec3d direccion_brazo = jBrazoIzq.dirsegmento * jBrazoIzq.trf->getMatrix(); // referido al Cuerpo
    direccion_brazo = direccion_brazo - osg::Vec3d(20.0f,0.0f,82.0f); // resta origen para referir al Hombro
    double alfa = atan2(-direccion_brazo.y(),direccion_brazo.z());
    jHombroIzq.trf->setMatrix(osg::Matrix::rotate(osg::inDegrees(-90.0f),0.0f,0.0f,1.0f) *
			      osg::Matrix::rotate(alfa-PI/2,1.0f,0.0f,0.0f) *
			      osg::Matrix::translate(20.0f,0.0f,82.0f));    
  }
  // Función para mover el hombro a partir del giro en X del brazo, relativo al plano sagital (inicial)
  void modeloNOMAD::mueveHombroDer()
  {
    Vec3d direccion_brazo = jBrazoDer.dirsegmento * jBrazoDer.trf->getMatrix(); // referido al Cuerpo
    direccion_brazo = direccion_brazo - osg::Vec3d(-20.0f,0.0f,82.0f); // resta origen para referir al Hombro
    double alfa = atan2(-direccion_brazo.y(),direccion_brazo.z());
    jHombroDer.trf->setMatrix(osg::Matrix::rotate(osg::inDegrees(90.0f),0.0f,0.0f,1.0f) *
			      osg::Matrix::rotate(alfa-PI/2,1.0f,0.0f,0.0f) *
  ...
  // Función para obtener alternativas a la posición del codo derecho, para el NOMAD-1.
  ...
  std::vector<osg::Vec3d>  modeloNOMAD::alternativasBrazoDer(int opciones, double angmax)
  {
  ...
  // Función para obtener alternativas a la posición del codo izquierdo, para el NOMAD-1.
  // ENTRADAS: 
  //   int opciones - Número de alternativas que va a ofrecer la función
  ...
  //   posición de referencia a las más alejadas
  std::vector<osg::Vec3d>  modeloNOMAD::alternativasBrazoIzq(int opciones, double angmax)
  {
    std::vector<osg::Vec3d> opc(opciones);
    bool par = true;
    double theta;
  ...
  // la mano (Persona)
  std::vector<double> modeloNOMAD::apuntaBrazoDer(const osg::Vec3d& codo, const osg::Vec3d& donde)
  {
    // Matrices de resultados parciales
    osg::Matrix homb;
    osg::Matrix cod;
  ...
  // la mano (Persona)
  std::vector<double> modeloNOMAD::apuntaBrazoIzq(const osg::Vec3d& codo, const osg::Vec3d& donde)
  {
    // Matrices de resultados parciales
    osg::Matrix homb;
    osg::Matrix cod;
  ...
  void modeloNOMAD::giraBrazoIzq(const std::vector<double> & radianes)
  {
  ...
  void modeloNOMAD::giraBrazoDer(const std::vector<double> & radianes)
  {
    assert(radianes.size() == 4);
  ...
  bool modeloNOMAD::mueveCabeza(const osg::Vec3d& cabeza)
  {
  ...
  bool modeloNOMAD::alineaHombros(const osg::Vec3d& hombroizq, const osg::Vec3d& hombroder)
  {
  ...
  bool modeloNOMAD::mueveBrazoIzq(const osg::Vec3d& manoizq, const osg::Vec3d& codoizq)
  {
    int ki;
	...
	/*alternatas = this->alternativasBrazoIzqNOMAD (alter);   
    ...
    if ( (par_colisionado!=-1)||(fuera_limites) ) // Si la posición no es válida...
      {
	cout<<"Buscando alternativzs"<<endl;
    ...
    // Rota el hombro en el plano sagital
    mueveHombroIzq();
...
//RETARGETING ANGULOS
 std::vector<double> modeloNOMAD::mueveBrazoIzq(const osg::Vec3d& manoizq,std::vector<double> angulos_brazo_izq,std::vector<double> ang_left_pos_ant, const osg::Vec3d& codoizq)
 {
    int ki;
    bool angulo_valido[4];
    // NUEVO 
...
//FIN RETARGETING ANGULOS
  ...
  bool modeloNOMAD::mueveBrazoDer(const osg::Vec3d& manoder, const osg::Vec3d& cododer)
  {
    int ki;
    bool angulo_valido[4];
    ...
    MatrixTransform* ultimaBrazo = new MatrixTransform;
    MatrixTransform* ultimaAntebrazo = new MatrixTransform;
    MatrixTransform* ultimaHombro = new MatrixTransform;
    ...
    ultimaHombro->setMatrix( this->jHombroDer.trf->getMatrix() );
    for (ki=0; ki<4; ki++)
      {
	angulo_valido[ki] = true;
	...
	par_colisionado = -1;
	// PRUEBA. TAMBIEN SACA ALTERNATIVAS, PA VER... PERO LAS SACA CON BOLAS 3D Y TO!!!! OOOOOMA MORENOOOOOO!!!
	/*alternatas = this->alternativasBrazoDerNOMAD (alter);   
	  for (int kk=0;kk<alter;kk++)
	  {
	  if (primen){
	  bolas[kk] = mostrarAlternativa(alternatas[kk]);
	...
	if (!encuentra_alternativa)
	  {
		...
		this->jHombroDer.trf->setMatrix( ultimaHombro->getMatrix() );
	      }
	  }
    ...
    // Se mueve el hombro, solidario con el giro en el plano sagital del brazo
    mueveHombroDer();
...
std::vector<double> modeloNOMAD::mueveBrazoDer(const osg::Vec3d& manoder,std::vector<double> angulos_brazo_der,std::vector<double> ang_right_pos_ant, const osg::Vec3d& cododer)
  {
    int ki;
    bool angulo_valido[4];
...
//FIN RETARGETING ANGULOS
  ...
  void modeloNOMAD::setAlturaCara(int altura_cm)
  {
  ...
  int modeloNOMAD::devuelveAltura()
  {
    double altura = jCuerpo.dirsegmento.length() + jBase.dirsegmento.length() + jCabeza.dirsegmento.length()/2;
    
    return( (int)(altura/10) );   // Se divide entre 10 porque las medidas del NOMAD están orig. en mm.
  }

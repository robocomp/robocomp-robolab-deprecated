...
#ifndef MODELOVISORpalotes_H
#define MODELOVISORpalotes_H
...
namespace osgDTE {
  ...
  class modeloVISORpalotes: public modelo {
    
  public:
    
    // Constructores
    modeloVISORpalotes() {_pobs = NULL; _obs = NULL; crearsegmentos();_scale = 1.0; particula = 0.0f;
                   ang_left_arm.resize(4,0.0);ang_right_arm.resize(4,0.0);
		   numero_alternativas = 20; angmax = PI/2; 
		   alternatas_izq.resize(numero_alternativas);
		   alternatas_der.resize(numero_alternativas);}
    modeloVISORpalotes(osgDTE::observadormodelocoli* pobs) {_pobs = pobs; _obs = NULL; crearsegmentos(); _scale = 1.0;
                ang_left_arm.resize(4,0.0);ang_right_arm.resize(4,0.0);
		numero_alternativas = 20; angmax = PI/2; 
		alternatas_izq.resize(numero_alternativas);
		alternatas_der.resize(numero_alternativas);}

 ...
 */
void CameraBusI::getImageAndStates(const string &camera,const Format &format, Image& image, RoboCompCommonHead::THeadState &hState,RoboCompDifferentialRobot::TBaseState &bState, const Ice::Current& )
{
 ...
 * @return TBaseState Struct contains base position compute by odometry
 */
void CameraBusI::getSyncImagesAndStates(const CameraList& cameraList, const Format& format, bool all, ImageList& imagelist, RoboCompCommonHead::THeadState &hState,RoboCompDifferentialRobot::TBaseState &bState, const Ice::Current & )
{
	worker->getBaseState(bState);
	worker->getHeadState(hState);
	worker->getSyncImages(cameraList,format,all,imagelist);

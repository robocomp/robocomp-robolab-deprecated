...
using namespace RoboCompEvaluadorOff;
using namespace RoboCompUrsusArm;
using namespace RoboCompSpeechgenerator;
using namespace RoboCompSpeech;
using namespace RoboCompTrackeropenni;


class juegoComp : public RoboComp::Application
{
private:
...
void juegoComp::initialize()
{
	// Config file properties read example
	// configGetString( PROPERTY_NAME_1, property1_holder, PROPERTY_1_DEFAULT_VALUE );
	// configGetInt( PROPERTY_NAME_2, property1_holder, PROPERTY_2_DEFAULT_VALUE );
}
int juegoComp::run(int argc, char* argv[])
{
// #ifdef USE_QTGUI
...
#endif*/
	a.setApplicationName("juego");
	int status=EXIT_SUCCESS;

	// Remote server proxy access example
	// RemoteComponentPrx remotecomponent_proxy;
	TrackeropenniPrx trackeropenni_proxy;
EvaluadorOffPrx evaluadoroff_proxy;
SpeechPrx speech_proxy;
UrsusArmPrx ursusarm_proxy;
SpeechgeneratorPrx speechgenerator_proxy;
	...
	try
	{
		evaluadoroff_proxy = EvaluadorOffPrx::uncheckedCast( communicator()->stringToProxy( getProxyString("EvaluadorOffProxy") ) );
	}
	catch(const Ice::Exception& ex)
	{
	...
	rInfo("EvaluadorOffProxy initialized Ok!");
	mprx["EvaluadorOffProxy"] = (::IceProxy::Ice::Object*)(&evaluadoroff_proxy);//Remote server proxy creation example
	try
	{
		ursusarm_proxy = UrsusArmPrx::uncheckedCast( communicator()->stringToProxy( getProxyString("UrsusArmProxy") ) );
	}
	catch(const Ice::Exception& ex)
	{
		cout << "[" << PROGRAM_NAME << "]: Exception: " << ex;
	...
	rInfo("UrsusArmProxy initialized Ok!");
	mprx["UrsusArmProxy"] = (::IceProxy::Ice::Object*)(&ursusarm_proxy);//Remote server proxy creation example
	try
	{
		speechgenerator_proxy = SpeechgeneratorPrx::uncheckedCast( communicator()->stringToProxy( getProxyString("SpeechgeneratorProxy") ) );
	}
	catch(const Ice::Exception& ex)
	{
		cout << "[" << PROGRAM_NAME << "]: Exception: " << ex;
	...
	rInfo("SpeechgeneratorProxy initialized Ok!");
	mprx["SpeechgeneratorProxy"] = (::IceProxy::Ice::Object*)(&speechgenerator_proxy);//Remote server proxy creation example
	try
	{
	...
	try
	{
		trackeropenni_proxy = TrackeropenniPrx::uncheckedCast( communicator()->stringToProxy( getProxyString("TrackeropenniProxy") ) );
	}
	catch(const Ice::Exception& ex)
	{
		cout << "[" << PROGRAM_NAME << "]: Exception: " << ex;
	...
	rInfo("TrackeropenniProxy initialized Ok!");
	...
	try
	{
		...
		// Server adapter creation and publication
		Ice::ObjectAdapterPtr adapterJuego = communicator()->createObjectAdapter("JuegoComp");
		JuegoI *juego = new JuegoI(worker);
		adapterJuego->add(juego, communicator()->stringToIdentity("juego"));

		adapterJuego->activate();
